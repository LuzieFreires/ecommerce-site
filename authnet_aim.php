	...
	private function process($retries = 3) {
        ...
        while ($count < $retries) {
            ...
            if ($this->response === false) {
                throw new AuthnetARBException('CURL Error: ' . curl_error($ch));
            }
            
            $this->_parseResults();
            
            if (empty($this->results) || count($this->results) < 7) {
                throw new AuthnetARBException('Invalid response from payment gateway');
            }
